...
export const analyzeImage = async (base64Image: string): Promise<AnalysisResult> => {
  try {
    ...
    const textPart = {
      text: "You are a highly accurate shopping assistant. Your primary goal is to find REAL, purchasable products with REAL images. First, analyze the main object in the image and provide its common name and estimated average price in USD. Then, for each of the following sellers, find 2 or 3 REAL, visually and functionally similar items that actually exist: amazon, ebay, walmart, target, bestbuy, etsy, aliexpress, alibaba. CRITICAL: For each recommended item, you MUST provide: 1) A precise product name of a REAL product that exists, 2) Its realistic average price range in USD (e.g., '$120 - $150'), 3) A REAL, working image URL from the actual product listing (NOT placeholder images), and 4) A category from: 'clothing', 'shoes', 'electronics', 'book', 'food', 'drinkware', 'furniture', 'tool', 'toy', 'generic'. DO NOT use placeholder images, stock photos, or fake URLs. Only recommend products that actually exist with real images. If you cannot find real products for a seller, provide an empty array. Your response must be ONLY the JSON object matching the schema.",
    };
